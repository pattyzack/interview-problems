...
public class PetLoader implements InitializingBean {
    ...
    private void loadBreed(String breed, Resource source) throws IOException {
        // description, filename, file, uri, url

        System.out.println("Loading " + breed);
        try (BufferedReader br = new BufferedReader(new InputStreamReader(source.getInputStream()))) {
            String line;
            while ((line = br.readLine()) != null) {
                System.out.println(line);
                ImageEntity entity = new ImageEntity(null, breed, source.getDescription(), source.getFilename(), source.getURI().toString(), source.getURL().toString(), 0l, line);
                ImageEntity savedEntity = imageEntityRepository.saveAndFlush(entity);
                //System.out.println("Saved entity: " + savedEntity.toString());
            }
        }
    }
}
